...
const linkClass = ({ isActive }: { isActive: boolean }) =>
  [
    ...
    isActive ? 'bg-[#1a56db] text-white shadow-[0_8px_20px_rgba(26,86,219,0.18)]' : 'text-[#434654] hover:bg-white hover:text-[#003fb1]',
  ].join(' ')
...
export function Sidebar() {
  return (
    <aside className="hidden min-h-screen flex-col bg-[#faf8ff] px-5 py-6 lg:flex">
      <div className="mb-10 flex items-center gap-3 px-1">
        <div className="flex h-11 w-11 items-center justify-center rounded-lg bg-[#dbe1ff] text-[#003fb1]">
          <PackageSearch className="h-6 w-6" />
        </div>
        <div>
          <h2 className="text-xl font-bold text-[#003fb1]">FoundTrust</h2>
          <p className="text-sm font-medium text-[#737686]">Operational Portal</p>
        </div>
      </div>
